...
# The links in the list are some of the items I currently own
...
def find_price():
    string = ""                                              # Holds an empty string that will be formatted
    for item in links:
        ...
        # name is currently formatted like "[H] Titanium White Octane [W] Credits"
        name = name.replace("[H]", "")
        name = name.replace("[W]", "")
...
if __name__ == "__main__":
    while True:
        find_price()                                         # Calls the function
        time.sleep(1000)                                     # Time in seconds until it runs the method again
